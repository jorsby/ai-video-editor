  ...
  if (candidates.length === 0) {
    for (const item of items) {
      result.set(item.gridPosition, {
        matched: false,
        confidence: 0,
        reason: 'No reusable video assets found for this type.',
        strategy: 'none',
      });
    }
    return result;
...
Match requested ${itemType}s to existing video assets.
...
- reason (short, concrete)`;
  ...
  try {
    ...
    console.warn('[video-asset-ai-matcher] AI match request failed:', message);
