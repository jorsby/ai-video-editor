...
export async function generateCaptionClips(
  ...
  const fontFamily =
    options.fontFamily ?? (isRTL ? 'Cairo' : 'Bangers-Regular');
  const fontUrl =
    options.fontUrl ??
    (isRTL
  ...
  for (const chunk of captionChunks) {
    ...
    clips.push({
      ...
      top: (() => {
        ...
        if (align === 'top-quarter')
          return videoHeight * 0.3 - captionHeight / 2;
        if (align === 'center') return (videoHeight - captionHeight) / 2;
        if (align === 'bottom-quarter')
          return videoHeight * 0.7 - captionHeight / 2;
