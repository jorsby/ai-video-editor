...
export async function POST(req: NextRequest) {
  ...
  if (!storyboard_id || !target_language) {
    return NextResponse.json(
      { error: 'storyboard_id and target_language are required' },
      { status: 400 }
    );
  }
  ...
  const sourceLang =
    scenes[0].voiceovers?.find(
      (v: { status: string }) => v.status === 'success'
    )?.language ?? 'en';

  // Check target language doesn't already exist
  const alreadyExists = scenes[0].voiceovers?.some(
    (v: { language: string }) => v.language === target_language
  );
  if (alreadyExists) {
    return NextResponse.json(
      { error: `${target_language} voiceovers already exist` },
      { status: 409 }
    );
  }
  ...
  const segments = scenes.map(
    (scene: {
      voiceovers: { language: string; text: string | null; status: string }[];
    }) =>
      scene.voiceovers?.find(
        (v) => v.language === sourceLang && v.status === 'success'
      )?.text ?? ''
  );

  const targetLangName =
    SUPPORTED_LANGUAGES.find((l) => l.code === target_language)?.name ??
    target_language;
...
Return ONLY valid JSON: { "${target_language}": ["segment1", "segment2", ...] }`;
  ...
  const translationSchema = z.object({
    [target_language]: z.array(z.string()),
  });
  ...
  try {
    ...
    if (translated.length !== scenes.length) {
      return NextResponse.json(
        { error: 'Translation segment count mismatch' },
        { status: 500 }
      );
    }
