...
/**
 * POST /api/v2/scenes/{id}/generate-last-frame
 *
 * Uses scene.last_frame_prompt format:
 * BASE: @background-slug OR use_first_frame
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const { data: scene, error: sceneError } = await supabase
      .from('scenes')
      .select('id, episode_id, first_frame_url, last_frame_prompt')
      .eq('id', sceneId)
      .maybeSingle();
    ...
    if (!scene.last_frame_prompt?.trim()) {
      return NextResponse.json(
        {
          error:
            'Scene has no last_frame_prompt. Set last_frame_prompt before generating.',
        },
        { status: 400 }
      );
    }
    ...
    try {
      parsedPrompt = parseFramePrompt(scene.last_frame_prompt);
    } catch (error) {
      return NextResponse.json(
        {
          error:
            error instanceof Error
              ? error.message
              : 'Invalid last_frame_prompt format.',
        },
        { status: 400 }
      );
    }
    ...
    if (parsedPrompt.base.kind === 'use_first_frame') {
      if (!scene.first_frame_url) {
        return NextResponse.json(
          {
            error:
              'BASE: use_first_frame requires scene.first_frame_url to be present.',
          },
          { status: 400 }
        );
      }
      baseUrl = scene.first_frame_url;
    } else {
    ...
    webhookUrl.searchParams.set('step', 'GenerateLastFrame');
    ...
    const task = await createTask({
      ...
      input: {
        ...
        image_input: [baseUrl, ...resolvedRefs.urls],
    ...
    await supabase
      .from('scenes')
      .update({
        last_frame_status: 'generating',
        last_frame_task_id: task.taskId,
      })
      .eq('id', sceneId);
    ...
    console.error('[v2/scenes/:id/generate-last-frame] Error:', error);
    return NextResponse.json(
