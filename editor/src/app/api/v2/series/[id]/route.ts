...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    console.error('[v2/series/:id][GET] Unexpected error:', error);
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  try {
    ...
    if (error || !series) {
      console.error('[v2/series/:id][PATCH] Failed to update series:', error);
      return NextResponse.json(
    ...
    console.error('[v2/series/:id][PATCH] Unexpected error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
// DELETE /api/v2/series/{id}
// Hard-deletes a series and everything under it (cascade: assets → variants → episodes → scenes).
export async function DELETE(req: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      console.error('[v2/series/:id][DELETE] Failed to delete series:', error);
      return NextResponse.json(
    ...
    console.error('[v2/series/:id][DELETE] Unexpected error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
