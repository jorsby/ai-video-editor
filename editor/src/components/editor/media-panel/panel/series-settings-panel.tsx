...
export default function SeriesSettingsPanel() {
  const projectId = useProjectId();
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [settings, setSettings] = useState<SeriesSettings | null>(null);
  const [seriesId, setSeriesId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  // Draft state for editing
  const [draft, setDraft] = useState<Partial<SeriesSettings>>({});
  ...
  const load = useCallback(async () => {
    ...
    const resolvedSeriesId =
      typeof projectSettings?.series_id === 'string'
        ? projectSettings.series_id
        : null;

    if (!resolvedSeriesId) {
      setError('No series linked to this project');
      setIsLoading(false);
      return;
    }
    ...
    if (fetchError || !series) {
      setError('Failed to load series settings');
      setIsLoading(false);
      return;
    }

    setSeriesId(resolvedSeriesId);
    setSettings(series as SeriesSettings);
    setDraft({});
    setIsLoading(false);
  }, [projectId]);
  ...
  const updateDraft = (field: keyof SeriesSettings, value: unknown) => {
    setDraft((prev) => {
