...
export function WorkflowRunDetailDialog({
  ...
  const [refreshedLanes, setRefreshedLanes] = useState<
    Map<string, RefreshedLane>
  >(new Map());
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {run.schedule_type === 'scheduled' ? 'Scheduled' : 'Published'}:{' '}
            {dateLabel}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            {run.lanes.map((lane) => {
              ...
              const cachedPost = lane.mixpost_uuid
                ? postsById.get(lane.mixpost_uuid)
                : undefined;
              const viewPost = refreshed?.refreshedPost ?? cachedPost;
              const langLabel =
                LANG_LABEL[lane.language] ?? lane.language.toUpperCase();
          ...
          <div className="border-t border-border/50 pt-2 flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {run.timezone ?? 'UTC'} · {run.lanes.length}{' '}
              {run.lanes.length === 1 ? 'lane' : 'lanes'}
            </span>
            <Button
              variant="ghost"
              size="sm"
