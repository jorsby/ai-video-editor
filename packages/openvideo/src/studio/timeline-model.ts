...
export class TimelineModel {
  ...
  /**
   * Replace all clips matching a predicate with new clips generated by a factory.
   * Factory can return null to skip replacement (old clip is removed).
   */
  async replaceClipsByPredicate(
    predicate: (clip: IClip) => boolean,
    newClipFactory: (oldClip: IClip) => Promise<IClip | null>
  ): Promise<void> {
    const toReplace = this.clips.filter(predicate);
    if (toReplace.length === 0) return;
    ...
    for (const oldClip of toReplace) {
      ...
      if (newClip) {
        ...
        // 3. Replace in internal clips list
        const clipIdx = this.clips.indexOf(oldClip);
        if (clipIdx !== -1) {
          this.clips[clipIdx] = newClip;
        }
        ...
        // Factory returned null — remove the old clip
        const clipIdx = this.clips.indexOf(oldClip);
        if (clipIdx !== -1) {
        ...
        const trackClipIdx = track.clipIds.indexOf(oldClip.id);
        if (trackClipIdx !== -1) {
          track.clipIds.splice(trackClipIdx, 1);
        }
        this.studio.emit('clip:removed', { clipId: oldClip.id });
      }

      // 6. Cleanup old visuals and listeners
      const oldRenderer = this.studio.spriteRenderers.get(oldClip);
      if (oldRenderer) {
        oldRenderer.destroy();
        this.studio.spriteRenderers.delete(oldClip);
      ...
      // 7. Cleanup old clip from interactive set
      this.studio.selection.interactiveClips.delete(oldClip);

      // 8. Sync selection
      if (this.studio.selection.selectedClips.has(oldClip)) {
        this.studio.selection.selectedClips.delete(oldClip);
        if (newClip) this.studio.selection.selectedClips.add(newClip);
      }
  ...
  /**
   * Remove gaps between clips by shifting clips to the left.
   * Clip duration and trim are unchanged — only display.from / display.to move.
   * @param trackId - If provided, only collapse gaps on that track. Otherwise all tracks.
   */
  async collapseGaps(trackId?: string): Promise<void> {
    ...
    for (const track of targetTracks) {
      const trackClips = track.clipIds
        .map((id) => this.clips.find((c) => c.id === id))
        .filter((c): c is IClip => c !== undefined)
        .sort((a, b) => a.display.from - b.display.from);

      let cursor = 0;
      for (const clip of trackClips) {
        if (clip.display.from !== cursor) {
          updates.push({
            id: clip.id,
            updates: { display: { from: cursor, to: cursor + clip.duration } },
          });
        }
    ...
    if (updates.length > 0) {
      await this.updateClips(updates);
    }
  }
